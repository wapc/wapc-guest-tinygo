...
func (w *wapcHost) consoleLog(ctx context.Context, m api.Module, ptr, len uint32) {
	msg := w.requireReadString(m.Memory(), "msg", ptr, len)
...
func (w *wapcHost) requireReadString(mem api.Memory, fieldName string, offset, byteCount uint32) string {
	return string(w.requireRead(mem, fieldName, offset, byteCount))
}

// requireRead is like api.Memory except that it panics if the offset and byteCount are out of range.
func (w *wapcHost) requireRead(mem api.Memory, fieldName string, offset, byteCount uint32) []byte {
